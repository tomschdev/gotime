...
// The Build method returns a finished Notification object
func (nb *NotificationBuilder) Build() (*Notification, error) {
	// Error checking can be done at the Build stage
	if nb.Icon != "" && nb.SubTitle == "" {
		return nil, fmt.Errorf("Cannot specify icon without subtitle")
	}
	if nb.Priority > 5 || nb.Priority < 0 {
		return nil, fmt.Errorf("Cannot specify priority outside of 0 and 5 (incl)")
	}
